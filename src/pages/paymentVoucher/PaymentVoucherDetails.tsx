...
export const PaymentVoucherDetails = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await paymentVoucherService.deletePaymentVoucher(voucher.VoucherNo, voucher.CompanyID);
      if (result.success) {
  ...
  const handleSubmitForApproval = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(result.message || "Payment voucher submitted for approval");
  ...
  const handleApprovalAction = async () => {
    ...
    setActionLoading(true);
    try {
      const result = await paymentVoucherService.approveOrRejectPaymentVoucher(voucher.VoucherNo, voucher.CompanyID, approvalAction, approvalComments.trim() || undefined);

      if (result.success) {
        toast.success(result.message || `Payment voucher ${approvalAction.toLowerCase()}d successfully`);
        window.location.reload();
      } else {
        toast.error(result.message || `Failed to ${approvalAction.toLowerCase()} payment voucher`);
      }
    } catch (err) {
  ...
  const handleDownloadAttachment = (attachment: PaymentVoucherAttachment) => {
  ...
  const handleViewAttachment = (attachment: PaymentVoucherAttachment) => {
    if (attachment.fileUrl) {
  ...
  const canEdit = voucher.PaymentStatus === "Draft" || voucher.PaymentStatus === "Pending";
  const canDelete = voucher.PaymentStatus === "Draft" || voucher.PaymentStatus === "Pending";
  const canSubmit = voucher.PaymentStatus === "Draft";
  const canApprove = voucher.PaymentStatus === "Pending";
    ...
    <div className="container mx-auto p-4 space-y-6">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-lg font-semibold">{formatCurrency(voucher.TotalAmount, voucher.CurrencyName)}</span>
                </div>
                <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <span className="text-sm font-medium text-muted-foreground">Payment Account:</span>
                  <span>{voucher.PaymentAccountName || "—"}</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <span className="text-sm font-medium text-muted-foreground">Total Amount:</span>
                  <span className="font-semibold">{formatCurrency(voucher.TotalAmount, voucher.CurrencyName)}</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Debit entries for this payment</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {approvalAction === ApprovalAction.APPROVE
                ? `Are you sure you want to approve payment voucher "${voucher.VoucherNo}"?`
                : `Are you sure you want to reject payment voucher "${voucher.VoucherNo}"?`}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
