...
export function mapIconToComponent(iconName: string | null | undefined): LucideIcon {
  ...
  const directMatch = Object.keys(LucideIcons).find((key) => key.toLowerCase().replace(/icon$/, "") === normalizedName);
  ...
  const similarMatch = Object.keys(LucideIcons).find((key) => key.toLowerCase().includes(normalizedName));
