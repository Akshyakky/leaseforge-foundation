...
export function MultiStepForm<TFieldValues extends FieldValues>({
  ...
  return (
    <div className={className}>
      <Stepper currentStep={currentStep} steps={steps.map((step) => ({ id: step.id, title: step.title }))} onStepClick={handleStepClick} />
          ...
          <form onSubmit={form.handleSubmit(handleNext)}>
            <CardContent className="space-y-4">
              {currentStepData.fields
                ...
                .map((field) => (
                  <FormField key={String(field.name)} form={form} {...field} />
                ))}
            </CardContent>

            <CardFooter className="flex justify-between">
              <Button type="button" variant="outline" onClick={handlePrevious} disabled={isFirstStep || isLoading}>
